...
- **对话状态**：Open Items 基本完成，待规划执行阶段拆分
...
1. **规划执行阶段拆分** — 用户要求合理拆分工作量，每步一个 agent，含纠偏机制，可回退。当前 stages/README.md 的 14 步方案需要重新规划
2. **新建 `triattention` conda 环境** — 基于 rkv 环境版本，cover 除 GPT-OSS 外所有模型
3. **进入代码清理执行** — 按新规划的阶段逐步推进
