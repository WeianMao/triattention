...
def _apply_v2_env(args: argparse.Namespace) -> None:
    ...
    os.environ["TRIATTN_V2_ENABLE_EXPERIMENTAL_KV_COMPACTION"] = str(
        args.enable_experimental_kv_compaction
    ).lower()
    os.environ["TRIATTN_V2_ENABLE_EXPERIMENTAL_BLOCK_RECLAIM"] = str(
        args.enable_experimental_block_reclaim
    ).lower()
    os.environ["TRIATTN_V2_REQUIRE_TRITON_SCORING"] = str(
        args.require_triton_scoring
    ).lower()
    os.environ["TRIATTN_V2_REQUIRE_PHYSICAL_RECLAIM"] = str(
        args.require_physical_reclaim
    ).lower()
    os.environ["TRIATTN_V2_FAIL_ON_EFFECTIVE_LEN_REGRESSION"] = str(
        args.fail_on_effective_len_regression
    ).lower()
    os.environ["TRIATTN_V2_EFFECTIVE_LEN_REGRESSION_RATIO"] = str(
        args.effective_len_regression_ratio
    )
    os.environ["TRIATTN_V2_EFFECTIVE_LEN_GUARD_DIVIDE_MULTIPLES"] = str(
        args.effective_len_guard_divide_multiples
    )
    os.environ["TRIATTN_V2_SCORE_CHUNK_MAX_TOKENS"] = str(
        args.score_chunk_max_tokens
    )
    os.environ["TRIATTN_V2_LOG_DECISIONS"] = str(args.log_decisions).lower()
    os.environ["TRIATTN_V2_WINDOW_SIZE"] = str(args.window_size)
    os.environ["TRIATTN_V2_PRUNING_MODE"] = str(args.pruning_mode)
    os.environ["TRIATTN_V2_SPARSE_SCORE_AGGREGATION"] = str(args.sparse_score_aggregation)
    os.environ["TRIATTN_V2_SPARSE_NORMALIZE_SCORES"] = str(args.sparse_normalize_scores).lower()
    os.environ["TRIATTN_V2_INCLUDE_PREFILL_IN_BUDGET"] = str(args.include_prefill_in_budget).lower()
    ...
    if args.sparse_stats_path:
        os.environ["TRIATTN_V2_SPARSE_STATS_PATH"] = str(resolve_path(args.sparse_stats_path))
    else:
        os.environ.pop("TRIATTN_V2_SPARSE_STATS_PATH", None)
    ...
    for run_id in run_ids:
        ...
        with out_path.open("a") as fout:
            ...
            for local_idx, prompt in enumerate(local_prompts):
                ...
                record["backend"] = "vllm_triattention_v2"
