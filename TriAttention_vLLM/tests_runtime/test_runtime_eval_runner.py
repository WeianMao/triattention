...
from evaluation.runner.vllm_triattention_runtime_runner import (
    _apply_runtime_env,
    compute_local_runs,
    setup_vllm_engine,
)
...
def _fake_integration_monkeypatch():
    ...
    fake = ModuleType("triattention_runtime.integration_monkeypatch")
    ...
    old = sys.modules.get("triattention_runtime.integration_monkeypatch")
    sys.modules["triattention_runtime.integration_monkeypatch"] = fake
    try:
        yield captured
    finally:
        if old is None:
            sys.modules.pop("triattention_runtime.integration_monkeypatch", None)
        else:
            sys.modules["triattention_runtime.integration_monkeypatch"] = old
...
def _args(disable_compression: bool) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        log_decisions=True,
        enforce_eager=False,
        force_runtime_integration=False,
        force_runtime_worker=False,
        force_runtime_scheduler=False,
    )


def test_apply_runtime_env():
    args = _args(disable_compression=False)
    with _patched_environ():
        _apply_runtime_env(args)
        assert os.environ["TRIATTN_RUNTIME_KV_BUDGET"] == "2048"
        assert os.environ["TRIATTN_RUNTIME_DIVIDE_LENGTH"] == "128"
        assert os.environ["TRIATTN_RUNTIME_PROTECT_PREFILL"] == "true"
        assert os.environ["TRIATTN_RUNTIME_ENABLE_EXPERIMENTAL_KV_COMPACTION"] == "true"
        assert os.environ["TRIATTN_RUNTIME_ENABLE_EXPERIMENTAL_BLOCK_RECLAIM"] == "true"
        assert os.environ["TRIATTN_RUNTIME_REQUIRE_TRITON_SCORING"] == "true"
        assert os.environ["TRIATTN_RUNTIME_REQUIRE_PHYSICAL_RECLAIM"] == "true"
        assert os.environ["TRIATTN_RUNTIME_FAIL_ON_EFFECTIVE_LEN_REGRESSION"] == "true"
        assert os.environ["TRIATTN_RUNTIME_EFFECTIVE_LEN_REGRESSION_RATIO"] == "0.9"
        assert os.environ["TRIATTN_RUNTIME_EFFECTIVE_LEN_GUARD_DIVIDE_MULTIPLES"] == "2"
        assert os.environ["TRIATTN_RUNTIME_PRUNING_MODE"] == "per_head"
        assert os.environ["TRIATTN_RUNTIME_PER_HEAD_SELECTION_SEMANTICS"] == "legacy_layer_local"
        assert os.environ["TRIATTN_RUNTIME_LAYER_PERHEAD_AGGREGATION"] == "max"
        assert os.environ["TRIATTN_RUNTIME_PER_LAYER_AGGREGATION"] == "max"
        assert os.environ["TRIATTN_RUNTIME_ALLOW_PER_LAYER_MODE"] == "false"
...
def test_setup_vllm_engine_force_runtime_scheduler_only_installs_scheduler_patch():
    args = _args(disable_compression=True)
    args.force_runtime_scheduler = True
    with _patched_environ(), _fake_vllm_module() as captured, _fake_integration_monkeypatch() as mp:
        setup_vllm_engine(args)
        assert captured["enforce_eager"] is False
        assert mp["patch_scheduler"] is True
