...
## 10. vLLM 集成方案更新 (2026-02-03)

### 10.1 集成方式确认

经调查确认，vLLM 的 attention backend 配置方式：

**配置方式**：
- ✅ 命令行参数：`--attention-backend <backend_name>`
- ✅ Python API：`LLM(attention_backend="<backend_name>", ...)`
- ❌ **不存在** `VLLM_ATTENTION_BACKEND` 环境变量

### 10.2 两种集成方案

**方案 A（主力）- 继承 FlashAttentionImpl**：
- 继承 `vllm.attention.backends.flash_attn.FlashAttentionImpl`
- 注册到 vLLM backend registry
- 通过 `--attention-backend triattention` 启用
- 优点：利用官方扩展机制，代码优雅，版本兼容性好

**方案 B（备用）- Monkey Patching**：
- 运行时替换 attention 方法
- 通过 `patch_vllm_attention(model, wrapper)` 启用
- 优点：快速实验，无需修改 vLLM 配置
- 定位：backup 方案，主力推进方案 A

### 10.3 分阶段实施

**Phase 1 - Attention 接口**：
- 当前阶段，仅处理 Attention 计算
- 压缩触发基于 `divide_length`

**Phase 2 - Scheduler 接口**：
- 下一阶段，实现内存触发压缩
