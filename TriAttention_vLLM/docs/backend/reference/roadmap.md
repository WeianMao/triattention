...
|-----|------|-----|------|
| 1.1 | 建立目录结构 | - | ✅ 完成 |
| 1.2 | 实现 `TriAttentionConfig` | 1.1 | ✅ 完成 |
| 1.3 | 实现状态/统计加载（`CompressionState`、stats loader） | 1.2 | ✅ 完成 |
| 1.4 | 实现 Triton 打分 kernel（基础版本） | 1.3 | ✅ 完成（含相位计算修正） |
| 1.5 | 使用 `torch.topk/torch.gather` 跑通 TopK/Gather | 1.4 | ✅ 完成 |
| 1.6 | 集成到 `compressor.py` 并完成正确性测试 | 1.5 | ✅ 完成（33/33 kernel tests passed） |
| 1.7 | vLLM 基础集成（PagedAttention 主路径） | 1.6 | ⏸️ 进行中 |
| 1.8 | 基线性能验证（非极限优化） | 1.7 | ⏸️ 待开始 |
