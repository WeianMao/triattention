# TriAttention_vLLM Runtime 方案总览
...
> 新的最终架构定稿见 `backend/RUNTIME_FINAL_ARCHITECTURE.md`，后续重构执行计划见 `interface/RUNTIME_REFACTOR_EXECUTION_PLAN_2026-02-22.md`。
>
> 2026-02-23 补充（执行调整）：当前主线目标模式仅保留 `per_head` / `per_layer_per_head`；`per_layer` 不作为交付目标或中间收敛态。压缩主线采用低搬运 fill-hole，运行时允许使用薄 patch/adapter（不可继续 patch-heavy）。详见 `interface/RUNTIME_SCHEME_ADJUSTMENT_2026-02-23.md`。
...
因此 Runtime 明确：**Attention 只做 attention 计算，不做 KV 管理决策**。

---

## 3. Runtime 接入架构（非侵入式优先，但不教条）
...
3. `--attention-backend`: 继续使用标准 FlashAttention（runtime 默认不自定义压缩后置逻辑）
...
Runtime 当前代码已实现大量原型与实验能力，但并非最终稳定形态。尤其以下路径已被识别为“需重构”的过渡方案：

1. `triattention_runtime/gpu_seq_len_patch.py`（worker 输入准备热路径补丁）
2. `triattention_runtime/hook_impl.py`（职责过载的大型编排函数）

后续主线将采用 `backend/RUNTIME_FINAL_ARCHITECTURE.md` 定义的“三层分离”：
...
- `TriAttention_vLLM/triattention_runtime/`（Runtime 开发目录）
...
- 每层独立执行 per-head TopK；用于复现早期 Runtime 结果（含约 45% 的历史锚点实验）。
