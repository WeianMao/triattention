...
3. Worker/Runner 执行编排
   ...
   - `TriAttention_vLLM/triattention_runtime/runner_compression_actions.py`
...
5. 布局整理与压缩/回收
   ...
   - `TriAttention_vLLM/triattention_runtime/kv_group_resolver.py`
...
6. 运行时输入语义适配（effective len / positions / slot mapping）
   ...
   - `TriAttention_vLLM/triattention_runtime/input_patch_backend.py`
   - `TriAttention_vLLM/triattention_runtime/input_patch_vllm_backend.py`
   - `TriAttention_vLLM/triattention_runtime/input_patch_ops.py`

## 3. 当前默认语义（重要）

1. 默认 `pruning_mode` 为 `per_head`
   - 配置定义：`TriAttention_vLLM/triattention_runtime/config.py:48`
2. `per_layer` 默认禁止误用（需显式 opt-in）
   - 校验：`TriAttention_vLLM/triattention_runtime/config.py:204`
   - selector 门禁：`TriAttention_vLLM/triattention_runtime/selector_hf.py:30`
3. 当前主线对齐语义：
   - `per_head`（HF 对齐）
