...
| 目标 | 说明 | 优先级 | 状态 |
|-----|------|--------|------|
| **Triton 级别效率** | Phase 1 做基础 Triton 优化，先保证可用与正确 | P0 | ✅ 完成 |
| **Batch Size > 1** | 支持批量推理，提高吞吐量 | P0 | ⏸️ 待验证 |
| **独立开发** | 不依赖 R-KV 框架，独立代码库 | P0 | ✅ 完成 |
| **vLLM 集成** | 与 vLLM 0.15.x 非侵入式集成 | P0 | ✅ 完成 |
| **Reasoning Benchmark** | HuggingFace vs vLLM 精度对比 | P0 | ✅ 框架完成 |
| CUDA Graph 兼容 | 支持 CUDA Graph 优化 | P2 | Phase 2 |
...
**Phase 1 = 核心实现 + vLLM 集成（常见场景）**：
- ✅ 在 TriAttention_vLLM 目录内完成 Triton 版本核心实现
- ✅ vLLM attention hook 集成（`patch_vllm_attention()`）
- ✅ Reasoning benchmark 框架（HuggingFace vs vLLM 精度对比）
- ✅ 覆盖最常见的推理路径（稳定、常见场景）
- ⏸️ 端到端精度验证（benchmark 运行中）
...
- Triton TopK/Gather 优化（2-3x 性能目标）
