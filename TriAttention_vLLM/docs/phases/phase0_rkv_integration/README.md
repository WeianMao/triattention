...
**核心原则**：在 R-KV 的 vLLM 上开发时，**不能修改他人的代码**，必须通过 ARG（命令行参数）实现隔离开发。

#### 2.3.1 允许的操作

| 操作 | 示例 | 说明 |
|-----|------|------|
| 添加新的 ARG 参数 | `--speckv-mode`, `--use-speckv` | 用于启用/配置 SpeckV |
| 在 ARG 分支内添加代码 | `if args.use_speckv: ...` | SpeckV 专属逻辑 |
| 新建独立文件 | `weian_development/speckv/*.py` | SpeckV 实现代码 |
| 新建独立脚本 | `weian_script/speckv/*.sh` | SpeckV 运行脚本 |

#### 2.3.2 禁止的操作

| 禁止操作 | 原因 |
|---------|------|
| 修改 `rkv/` 目录下的核心文件 | 会影响 R1KV、SnapKV、H2O 等其他算法 |
| 修改 `HuggingFace/rkv/` 目录 | 会影响框架基础功能 |
| 在非 ARG 分支内添加代码 | 会改变默认行为，影响他人实验 |
| 删除或重命名现有函数/类 | 会破坏向后兼容性 |

#### 2.3.3 隔离开发示例

**正确做法**：
```python
# 在 rkv_sharded_eval.py 中
parser.add_argument("--use-speckv", action="store_true")  # 新增 ARG

# 在执行逻辑中
if args.use_speckv:
    # SpeckV 专属路径，不影响其他算法
    from weian_development.speckv import apply_speckv_patch
    apply_speckv_patch(model, config)
else:
    # 原有逻辑完全不变
    pass
```
...
**错误做法**：
```python
# ❌ 直接修改 rkv/compression/base.py
class BaseCompressor:
    def update_kv(self, ...):
        # 添加 SpeckV 逻辑 ← 错误！会影响所有压缩器
```
...
```bash
# 1. 不带 SpeckV 参数运行，确认原有行为不变
bash weian_script/aime_sampled8/r1kv/run_r1kv_aime24.sh
# 应该与修改前结果完全一致

# 2. 带 SpeckV 参数运行，确认新功能正常
bash weian_script/aime_sampled8/speckv/aime24/run_speckv_aime24_perhead.sh
# SpeckV 功能正常工作
```
