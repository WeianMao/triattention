...
│   ├── COMPUTATION_OPTIMIZATION.md  # 计算优化
│   └── API.md                       # API 文档（后续）
├── triattention/
│   ├── __init__.py
...
│       └── compaction_kernel.py     # Triton KV 整理
├── integration/
│   ├── __init__.py
...
│   ├── test_scoring.py
│   ├── test_pruning.py
│   ├── test_paged_kv.py             # PagedAttention 测试
│   ├── test_performance.py
│   └── fixtures/
├── benchmarks/
...
| 2.3 | 实现 `compaction_kernel.py`（KV 整理） | 2.2 |
...
| 任务 | 优先级 |
|-----|-------|
| Stats 收集脚本 `collect_stats.py` | P1 |
| 多 GPU 支持 | P2 |
| 多 vLLM 版本测试 | P2 |
...
| 延迟开销 | < 10% |
| 稳定性 | 24h 零崩溃 |
| 模型覆盖 | LLaMA/Qwen/DeepSeek/Mistral/GPT-J/GPT-NeoX |
