"""Generate attention map visualization for Layer 3, Head 5."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate L3H5 attention map")
    parser.add_argument(
        "trace_dir",
        type=Path,
        nargs="?",
...
def generate_figure(
    ...
    mask_process_command("PD-L1_binder_l3h5")
    ...
    ax.set_title(f"L{layer}H{head} Attention Map", fontsize=FONT_SIZE, fontweight='bold')
...
def main() -> None:
    ...
    if args.output_path is None:
        ...
        output_path = output_dir / f"fig_l{args.layer}h{args.head}_attn_map.png"
    else:
