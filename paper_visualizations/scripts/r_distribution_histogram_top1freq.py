"""Generate R value distribution histogram for top1 dominant frequency.

For each head, selects the dominant frequency using amplitude product criterion,
then computes Mean Resultant Length R only for that single frequency.
"""
...
# ============ Mean Resultant Length for 1D complex ============
...
def compute_dominant_frequency(q_complex: torch.Tensor, k_complex: torch.Tensor) -> int:
    ...
    Returns the dominant frequency index.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate R distribution histogram (top1 frequency)")
    parser.add_argument(
        "trace_dir",
        type=Path,
        nargs="?",
...
def main() -> None:
    args = parse_args()
    mask_process_command("PD-L1_binder_r_hist_top1")

    if args.output_path is None:
        output_dir = Path("paper_visualizations/outputs/r_distribution")
        output_path = output_dir / "r_distribution_histogram_top1freq.png"
    else:
    ...
    # Compute R for all heads (top1 frequency only)
    r_q_values = []
    r_k_values = []

    print("Computing R values for all heads (top1 frequency)...")
    for layer in range(num_layers):
        for head in range(num_heads):
            ...
            # Extract values at dominant frequency
            q_vals = q_complex[:, dom_freq]
            k_vals = k_complex[:, dom_freq]

            # Compute R for this single frequency
