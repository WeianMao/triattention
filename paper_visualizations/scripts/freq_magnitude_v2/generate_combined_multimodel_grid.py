...
def main() -> None:
    ...
    for col, data in enumerate(model_data_list):
        ...
        # Use weights to convert counts to percentage
        weights = np.ones_like(all_pearson) * 100.0 / len(all_pearson)
        ax.hist(all_pearson, bins=bin_edges, weights=weights, color=color_recon, alpha=0.85, edgecolor='white', linewidth=0.8)
