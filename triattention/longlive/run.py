...
def main() -> None:
    ...
    if getattr(config, "lora_ckpt", None) and hasattr(pipeline.generator, "load_lora"):
        pipeline.generator.load_lora(config.lora_ckpt, config.adapter)
        pipeline.is_lora_enabled = True
        print(f"Loaded LoRA from {config.lora_ckpt}")
