...
# Lambda balance values (small weights as requested) and GPU assignments
...
run_single_experiment() {
    local lambda=$1
    local gpu=$2
    local exp_name="balance_lambda_${lambda//./_}"

    echo "[GPU $gpu] Starting lambda_balance=$lambda"

    # Create output directories
    mkdir -p output/ablation_balance/$exp_name/checkpoints
    mkdir -p output/ablation_balance/$exp_name/logs

    # Run training with specific lambda_balance and GPU
    # Note: lambda_activation=0.0 to isolate balance loss effect
...
# IMPORTANT: lambda_activation=0.0 to test only balance loss
config['training']['lambda_activation'] = 0.0
config['training']['lambda_balance'] = $lambda
config['output']['checkpoints_dir'] = 'output/ablation_balance/$exp_name/checkpoints'
config['output']['logs_dir'] = 'output/ablation_balance/$exp_name/logs'
...
results_path = Path('output/ablation_balance/$exp_name/results.json')
with open(results_path, 'w') as f:
    json.dump(results, f, indent=2, default=str)

logger.info('Completed lambda_balance=$lambda')
" 2>&1 | tee output/ablation_balance/$exp_name/run.log
...
wait
echo "All balance loss ablation experiments completed!"
