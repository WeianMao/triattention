"""
Training logic for Module 2 Multi-Bin Sparse Attention with Key-Balanced Loss.

Implements training loop with Key-Balanced Attraction Loss:
    - Original: loss = -log(sum_bins(p_q[b] * P[argmax_key, b])).mean()
    - Key-Balanced: loss = sum(weight_i * loss_i), where weight_i = (1/count_k) / sum(1/count_k)

This addresses the long-tail effect where some keys have many pairs while others have few.
Each unique key gets equal influence in the training gradient.
"""
...
def compute_attraction_loss(key_probs, query_bin_probs, argmax_keys, argmax_in_recent, eps=1e-8):
    """
    Compute Key-Balanced Attraction Loss for Module 2.

    Key-Balanced Loss addresses the long-tail effect where some keys have many pairs
    while others have few. Each unique key gets equal influence in training.

    Algorithm:
        1. Count occurrences of each unique key: count_k
        2. Compute raw weights: raw_weight_i = 1 / count_{k_i}
        3. Normalize weights: weight_i = raw_weight_i / sum(raw_weight_j)
        4. Weighted loss: loss = sum(weight_i * loss_i)
    ...
    # Per-pair loss: -log(match_prob)
    per_pair_loss = -torch.log(match_prob + eps)  # (num_valid,)

    # === Key-Balanced Weighting ===
    ...
    # Step 2: Compute raw weights (inverse of count for each pair)
    # counts[inverse_indices] maps each pair to its key's count
    raw_weights = 1.0 / counts[inverse_indices].float()  # (num_valid,)

    # Step 3: Normalize weights to sum to 1
    weights = raw_weights / raw_weights.sum()  # (num_valid,)
