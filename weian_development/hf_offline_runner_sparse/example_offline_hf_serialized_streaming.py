...
def run_sparse_generation(
    ...
    for _ in range(max_new_tokens):
        ...
        full_text = tokenizer.decode(
            sequence[0, prompt_length:],
            skip_special_tokens=True,
            clean_up_tokenization_spaces=False,
        )
        if len(full_text) > len(streamed_text):
            delta = full_text[len(streamed_text) :]
            print(delta, end="", flush=True)
            if stream_writer is not None:
                stream_writer.write(delta)
                stream_writer.flush()
    ...
    if stream_writer is not None and streamed_text:
        stream_writer.write("\n")
        stream_writer.flush()
...
def main() -> None:
    ...
    if args.stream_log_path is not None:
        stream_path = args.stream_log_path
        if not stream_path.is_absolute():
            stream_path = (Path.cwd() / stream_path).resolve()
        stream_path.parent.mkdir(parents=True, exist_ok=True)
        stream_writer = exit_stack.enter_context(stream_path.open("w", encoding="utf-8"))
