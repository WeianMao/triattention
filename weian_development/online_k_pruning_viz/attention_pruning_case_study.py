"""Compare baseline attention with frequency-based key pruning for selected heads."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize baseline attention vs. frequency-scored key pruning case studies.",
    )
    parser.add_argument(
        "input_root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Base directory for outputs (defaults to <input_root>/../attention_pruning_case_studies).",
    )
    parser.add_argument(
        "--target-size",
        type=int,
...
def compute_pruning_scores(
    ...
    for q_idx in range(seq_len):
        ...
        scores[q_idx, :valid_count] = (amp[:valid_count] * cos_phase).sum(dim=1)
    return scores
...
def main() -> None:
    ...
    if output_root is None:
        output_root = args.input_root.parent / "attention_pruning_case_studies"
    ...
    for layer, head_list in layer_to_heads.items():
        ...
        for head in head_list:
            ...
            amp, phi = compute_frequency_statistics(q_unrot, k_unrot)
            amp = amp.to(device=device, dtype=torch.float32)
            phi = phi.to(device=device, dtype=torch.float32)

            scores = compute_pruning_scores(amp, phi, omega)
            ...
            out_path = (
                output_root
                / f"layer_{layer:02d}_head_{head:02d}_pruning_comparison.png"
            )
