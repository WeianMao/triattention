"""Compare baseline attention with hybrid(0.9) frequency-based key pruning for selected heads."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize baseline attention vs. hybrid(0.9) frequency scoring key pruning case studies.",
    )
    parser.add_argument(
        "input_root",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Base directory for outputs (defaults to <input_root>/../attention_pruning_case_studies_hybrid09).",
    )
    parser.add_argument(
        "--target-size",
        type=int,
...
def compute_frequency_statistics(
    ...
    boost = (1.1 * q_abs_mean - q_mean_abs).clamp_min(0.0)
    extra = boost.unsqueeze(0) * k_abs
...
def main() -> None:
    ...
    if output_root is None:
        output_root = args.input_root.parent / "attention_pruning_case_studies_hybrid09"
    ...
    for layer, head_list in layer_to_heads.items():
        ...
        for head in head_list:
            ...
            out_path = (
                output_root
                / f"layer_{layer:02d}_head_{head:02d}_pruning_comparison_hybrid09.png"
            )
            ...
            if baseline_argmax is not None and pruned_argmax is not None:
                argmax_path = (
                    output_root
                    / f"layer_{layer:02d}_head_{head:02d}_pruning_argmax_hybrid09.png"
                )
                save_comparison_figure(
                    baseline_argmax,
                    pruned_argmax,
