# Top3 Frequency Band NMS Experiment
# Only consider top-3 frequency bands by energy in NMS algorithms
# Non-invasive copy of attention_pruning_case_study_nms_variance_isolated.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Number of top frequency bands to use for NMS (default: 6).",
    )
...
def get_top_k_frequency_indices(
    k_complex: torch.Tensor,
    top_k: int = 3,
) -> torch.Tensor:
    """Select top-K frequency bands by mean magnitude energy.

    Computes the mean complex vector across sequence dimension, then
    calculates magnitude to identify the highest-energy frequency bands.
    Used to filter frequency bands before NMS computation.

    Args:
        k_complex: [seq_len, freq_count] complex tensor of K vectors
        top_k: Number of top frequency bands to select (default: 3)

    Returns:
        top_indices: [top_k] tensor of frequency indices ranked by energy
    """
...
def incremental_variance_aware_nms(
    ...
    # Top-K Frequency Band Selection - only use highest energy frequencies
    top_indices = get_top_k_frequency_indices(new_k_complex, top_k=top_k)
    historical_k_complex = historical_k_complex[:, top_indices]  # Filter historical K
    new_k_complex = new_k_complex[:, top_indices]  # Filter new K
    w_low = w_low[top_indices]  # Filter weights
    w_high = w_high[top_indices]  # Filter weights
        ...
        if (
            ...
            if len(historical_indices) == 0:
                if len(new_indices) > 1:
                    ...
                    keep_mask = variance_aware_fast_nms(new_k_complex, w_low, w_high, top_k)
                ...
                hist_keep, new_keep = incremental_variance_aware_nms(
                    hist_k_complex, new_k_complex, w_low, w_high, top_k
                )
