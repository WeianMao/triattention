...
def score_keys_for_round_gaussian(
    ...
    quantile = mean_term + torch.sqrt(variance + 1e-12) * alpha
    combined = quantile.sum(dim=2)
...
def simulate_round_pruning_gaussian(
    k_real: torch.Tensor,
    k_imag: torch.Tensor,
    mu_x: torch.Tensor,
    mu_y: torch.Tensor,
    ...
    for q_idx in range(seq_len):
        if q_idx % round_window == 0:
            ...
            if keep_capacity <= 0:
                ...
                aggregated = score_keys_for_round_gaussian(
                    key_tensor,
                    q_idx,
                    k_real,
                    k_imag,
...
def main() -> None:
    ...
    for layer, head in sampled_heads:
        ...
        prune_mask = simulate_round_pruning_gaussian(
            k_real,
            k_imag,
            mu_x,
            mu_y,
