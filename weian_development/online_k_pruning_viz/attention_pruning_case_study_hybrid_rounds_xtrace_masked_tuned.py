"""Round-based hybrid key pruning with offset-aware high-frequency masking."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Base directory for outputs (defaults to <input_root>/../attention_pruning_case_studies_hybrid_rounds_xtrace_masked).",
    )
    parser.add_argument(
        "--stats-trace",
        type=Path,
    ...
    parser.add_argument(
        "--period-threshold-scale",
        type=float,
        default=0.5,
        help=(
            "Scale applied to delta when deciding whether to drop high-frequency components "
            "(period >= delta * scale keeps the component). Smaller values keep more frequencies."
        ),
    )
    parser.add_argument(
        "--mask-extra-term",
        action="store_true",
        help="If set, apply the same frequency mask to the additive (position-independent) extra term.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def score_keys_for_round(
    ...
    aggregation: str,
    period_threshold_scale: float,
    mask_extra_term: bool,
) -> torch.Tensor:
    ...
    delta_expanded = delta_grid.unsqueeze(2)
    omega_abs = omega.abs()
    safe_periods = torch.where(
        omega_abs > 0,
        (2 * math.pi) / omega_abs,
        torch.full_like(omega_abs, float("inf")),
    )
    period_grid = safe_periods.view(1, 1, -1)
    delta_threshold = delta_expanded * period_threshold_scale
    phase = delta_expanded * omega.view(1, 1, -1) + phi_sel.unsqueeze(1)
    # Mask high-frequency components whose periods fall below half of the
    # synthetic position (docs/attention_freq_randomk.md) to suppress distant noise.
    freq_mask = period_grid >= delta_threshold
    cos_phase = torch.cos(phase) * freq_mask.to(amp_sel.dtype)
    base_scores = (amp_sel.unsqueeze(1) * cos_phase).sum(dim=2)
    if mask_extra_term:
        extra_masked = (
            extra_sel.unsqueeze(1) * freq_mask.to(extra_sel.dtype)
        ).sum(dim=2)
        combined = base_scores + extra_masked
    else:
...
def main() -> None:
    ...
    if output_root is None:
        output_root = (
            args.input_root.parent / "attention_pruning_case_studies_hybrid_rounds_xtrace_masked"
        )
    ...
    for layer, head in sampled_heads:
        ...
        prune_mask = simulate_round_pruning(
            ...
            args.seed,
            args.period_threshold_scale,
            args.mask_extra_term,
        )
