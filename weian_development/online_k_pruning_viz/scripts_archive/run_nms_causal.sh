...
# Spectrum-Aware NMS Experiment: Causal Energy Method
# This script runs the pruning experiment WITH NMS using causal energy weights
...
OUTPUT_ROOT="$SCRIPT_DIR/results/nms_causal"
...
conda run -n dc python "$SCRIPT_DIR/attention_pruning_case_study_hybrid_rounds_xtrace_nms.py" \
    ...
    --round-window 64 \
    --nms-enabled \
    --energy-method causal \
    --verbose

echo "NMS Causal experiment completed. Results saved to: $OUTPUT_ROOT"
