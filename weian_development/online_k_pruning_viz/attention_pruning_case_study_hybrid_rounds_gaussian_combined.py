"""Round-based hybrid frequency key pruning using combined Gaussian head scores (Method 5.2)."""
...
from weian_development.hf_offline_runner_sparse.round_pruning_utils import invert_rope
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Right-tail probability used for the Gaussian top-p quantile score (0 < p < 1).",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def standard_normal_quantile(prob: float) -> float:
    if not 0.0 < prob < 1.0:
    ...
    value = dist.icdf(torch.tensor(float(prob), dtype=torch.float64))
    return float(value.item())


def compute_gaussian_frequency_stats(stats_unrot: torch.Tensor) -> GaussianFrequencyStats:
    seq_len, head_dim = stats_unrot.shape
    freq_count = head_dim // 2
    pairs = stats_unrot.view(seq_len, freq_count, 2).to(torch.float32)
    mean = pairs.mean(dim=0)
    centered = pairs - mean
    denom = max(seq_len - 1, 1)
    sigma_xx = (centered[..., 0] * centered[..., 0]).sum(dim=0) / denom
    sigma_xy = (centered[..., 0] * centered[..., 1]).sum(dim=0) / denom
    sigma_yy = (centered[..., 1] * centered[..., 1]).sum(dim=0) / denom
    eps = 1e-8
    sigma_xx = sigma_xx.clamp_min(eps)
    sigma_yy = sigma_yy.clamp_min(eps)
    return GaussianFrequencyStats(
        mean=mean.detach().cpu(),
        sigma_xx=sigma_xx.detach().cpu(),
        sigma_xy=sigma_xy.detach().cpu(),
        sigma_yy=sigma_yy.detach().cpu(),
    )


def split_key_components(k_unrot: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    seq_len, head_dim = k_unrot.shape
    freq_count = head_dim // 2
    pairs = k_unrot.view(seq_len, freq_count, 2)
    real = pairs[..., 0].contiguous()
    imag = pairs[..., 1].contiguous()
    return real, imag
...
def score_keys_for_round_gaussian_combined(
    ...
    if key_indices.numel() == 0:
        return torch.empty(0, device=k_real.device, dtype=torch.float32)

    device = k_real.device
    base_delta = (
        round_start - key_indices.to(device=device, dtype=torch.float32)
    )
    delta_grid = base_delta.unsqueeze(1) + offsets.unsqueeze(0)

    k_real_sel = k_real.index_select(0, key_indices)
    k_imag_sel = k_imag.index_select(0, key_indices)

    phase = delta_grid.unsqueeze(2) * omega.view(1, 1, -1)
    cos_phase = torch.cos(phase)
    sin_phase = torch.sin(phase)

    u = k_real_sel.unsqueeze(1) * cos_phase - k_imag_sel.unsqueeze(1) * sin_phase
    v = k_real_sel.unsqueeze(1) * sin_phase + k_imag_sel.unsqueeze(1) * cos_phase

    mu_x = mu_x.view(1, 1, -1)
    mu_y = mu_y.view(1, 1, -1)
    sigma_xx = sigma_xx.view(1, 1, -1)
    sigma_xy = sigma_xy.view(1, 1, -1)
    sigma_yy = sigma_yy.view(1, 1, -1)

    mean_term = mu_x * u + mu_y * v
    variance = (
        sigma_xx * (u ** 2)
        + 2.0 * sigma_xy * (u * v)
        + sigma_yy * (v ** 2)
    ...
    mean_total = mean_term.sum(dim=2)
    variance_total = variance.sum(dim=2)
    combined = mean_total + torch.sqrt(variance_total + 1e-12) * alpha
...
def simulate_round_pruning_gaussian_combined(
    k_real: torch.Tensor,
    k_imag: torch.Tensor,
    mu_x: torch.Tensor,
    mu_y: torch.Tensor,
    ...
    for q_idx in range(seq_len):
        if q_idx % round_window == 0:
            ...
            if keep_capacity <= 0:
                ...
                aggregated = score_keys_for_round_gaussian_combined(
                    key_tensor,
                    q_idx,
                    k_real,
                    k_imag,
...
def main() -> None:
    ...
    if output_root is None:
        output_root = (
            args.input_root.parent
            / "attention_pruning_case_studies_hybrid_rounds_gaussian_combined"
        )
    ...
    if not 0.0 < args.quantile_p < 1.0:
        raise SystemExit("--quantile-p must be in (0, 1)")
    alpha = standard_normal_quantile(1.0 - args.quantile_p)

    layer_to_heads_map: Dict[int, List[int]] = defaultdict(list)
    for layer, head in sampled_heads:
        layer_to_heads_map[layer].append(head)

    gaussian_cache: Dict[Tuple[int, int], GaussianFrequencyStats] = {}
    ...
    for layer, heads in layer_to_heads_map.items():
        ...
        for head in heads:
            ...
            stats_unrot = stats_unrot.to(torch.float32).contiguous()
            gaussian_cache[(layer, head)] = compute_gaussian_frequency_stats(stats_unrot)
        del stats_layer
        if device.type == "cuda":
    ...
    for layer, head in sampled_heads:
        ...
        _, k_unrot = invert_qk(q_head, k_head, cos_table, sin_table, attention_scale)

        k_unrot = k_unrot.to(torch.float32).contiguous()
        k_real, k_imag = split_key_components(k_unrot)
        k_real = k_real.to(device=device, dtype=torch.float32)
        k_imag = k_imag.to(device=device, dtype=torch.float32)

        stats = gaussian_cache[(layer, head)]
        mean = stats.mean.to(device=device, dtype=torch.float32)
        mu_x = mean[:, 0]
        mu_y = mean[:, 1]
        sigma_xx = stats.sigma_xx.to(device=device, dtype=torch.float32)
        sigma_xy = stats.sigma_xy.to(device=device, dtype=torch.float32)
        sigma_yy = stats.sigma_yy.to(device=device, dtype=torch.float32)

        prune_mask = simulate_round_pruning_gaussian_combined(
            k_real,
            k_imag,
            mu_x,
            mu_y,
    ...
    metrics = {
        ...
        "method": "gaussian_combined_quantile",
        "per_head": [
