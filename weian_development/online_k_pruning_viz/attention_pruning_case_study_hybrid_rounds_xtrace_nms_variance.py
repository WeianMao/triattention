"""Round-based hybrid frequency key pruning with variance-aware Q-magnitude weighted NMS.

This script implements variance-aware NMS using Q-magnitude percentile weights for
conservative weight selection. See docs/variance_aware_nms.md for algorithm details.

Key differences from spectrum-aware NMS (attention_pruning_case_study_hybrid_rounds_xtrace_nms.py):
- Uses Q-magnitude percentile weights (w_low, w_high) instead of frequency energy weights
- Implements conservative weight selection based on per-frequency score sign
- No normalization (epsilon=0 design)
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable variance-aware Q-magnitude weighted NMS before scoring each round.",
    )
    parser.add_argument(
        "--low-percentile",
        type=float,
...
def simulate_round_pruning(
    ...
    """Simulate round-based pruning with optional variance-aware NMS.

    Args:
        w_low: [F] low percentile Q-magnitude weights for NMS
        w_high: [F] high percentile Q-magnitude weights for NMS

    Returns:
        prune_mask: [seq_len, seq_len] bool tensor
        nms_drop_counts: List of NMS drop counts per round (empty if NMS disabled)
    """
    device = amp.device
    ...
    for q_idx in range(seq_len):
        if q_idx % round_window == 0:
            # Variance-aware NMS step: apply before scoring if enabled
            if nms_enabled and len(current_cache) > 1 and w_low is not None and w_high is not None and k_rotated_complex is not None:
                cache_indices = torch.tensor(current_cache, device=device, dtype=torch.long)
                # Extract K vectors for current cache
                k_cache_complex = k_rotated_complex[cache_indices]  # [cache_size, F]
                # Apply variance-aware NMS with conservative weight selection
                keep_mask = variance_aware_fast_nms(k_cache_complex, w_low, w_high)
                # Count dropped keys
                drop_count = (~keep_mask).sum().item()
                nms_drop_counts.append(int(drop_count))
                # Update cache with survivors
                surviving_indices = cache_indices[keep_mask].tolist()
                current_cache = surviving_indices
            else:
                nms_drop_counts.append(0) if nms_enabled else None
...
def main() -> None:
    ...
    # Cache for variance-aware NMS: stores (w_low, w_high) tuple per (layer, head)
    ...
    for layer, head in sampled_heads:
        ...
        # Load pre-computed Q-magnitude weights and prepare K complex for NMS if enabled
        w_low = None
        w_high = None
        k_rotated_complex = None
        if args.nms_enabled:
            # Convert K to complex pairs for NMS (using rotated K, i.e., original k_head)
            k_rotated_complex = to_complex_pairs(k_head)
            # Use pre-computed Q-magnitude percentile weights (computed at script start)
