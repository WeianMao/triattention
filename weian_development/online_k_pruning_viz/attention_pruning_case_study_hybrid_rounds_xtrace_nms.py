...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable spectrum-aware NMS before scoring each round.",
    )
    parser.add_argument(
        "--energy-method",
...
def simulate_round_pruning(
    ...
    """Simulate round-based pruning with optional NMS.
    ...
    for q_idx in range(seq_len):
        if q_idx % round_window == 0:
            # NMS step: apply before scoring if enabled
            if nms_enabled and len(current_cache) > 1 and freq_weights is not None and k_rotated_complex is not None:
                ...
                # Apply NMS
                keep_mask = fast_parallel_nms(k_cache_complex, freq_weights)
                # Count dropped keys
...
def main() -> None:
    ...
    freq_weights_cache: Dict[Tuple[int, int], torch.Tensor] = {}
        ...
        for head in heads:
            ...
            if args.nms_enabled:
                if args.energy_method == "amplitude":
                    ...
                    k_rotated_complex = to_complex_pairs(stats_k_head)
                    fw = compute_frequency_energy_weights(
                        q_rotated_complex, k_rotated_complex, "causal", device
                    )
                freq_weights_cache[(layer, head)] = fw.detach().cpu()
    ...
    for layer, head in sampled_heads:
        ...
        # Load pre-computed frequency weights and prepare K complex for NMS if enabled
        freq_weights = None
        ...
        if args.nms_enabled:
            ...
            # Use pre-computed frequency weights from stats_trace (computed at script start)
            freq_weights = freq_weights_cache[(layer, head)].to(device=device)
