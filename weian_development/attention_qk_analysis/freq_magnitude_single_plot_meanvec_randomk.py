...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Random seed used when sampling individual key tokens for diagnostics",
    )
...
def mean_vector_product_and_phase(
    ...
    return amp_product, phi_hat, q_mean, k_mean, k_complex
...
def plot_distance_curves(
    ...
    random_reconstructions: list[tuple[int, torch.Tensor]],
    out_path: Path,
    dpi: int,
    figsize: tuple[float, float],
) -> None:
    fig, axes = plt.subplots(2, 3, figsize=figsize, dpi=dpi, sharex=False)
    ...
    x_min = float(x_plain[0]) if x_plain.size else 1.0
    x_max = float(x_plain[-1]) if x_plain.size else x_min + 1.0
    ...
    for idx, ax in enumerate(bottom_axes):
        if idx < len(random_reconstructions):
            token_id, series = random_reconstructions[idx]
            values = series.detach().cpu().numpy()
            color = colors[idx % len(colors)]
            ax.plot(x_plain, values, color=color, linewidth=1.4)
            ax.set_title(f"Random key token {token_id}")
            ax.set_xlabel("Δ (tokens)")
            ax.set_xscale("log")
            ax.set_xlim(x_min, x_max)
            ax.grid(alpha=0.3, linestyle="--")
        else:
            ax.text(0.5, 0.5, "No sample", ha="center", va="center", fontsize=10)
            ax.set_xlabel("Δ (tokens)")
            ax.set_xscale("log")
            ax.set_xlim(x_min, x_max)
            ax.grid(alpha=0.3, linestyle="--")
...
def reconstruct_with_single_key(
    distance_vals: torch.Tensor,
    omega: torch.Tensor,
    q_mean: torch.Tensor,
    ...
    k_abs = torch.abs(k_vector)
    amp_product = (q_mean_abs * k_abs).to(dtype=distance_vals.dtype)

    relative = q_mean * torch.conj(k_vector)
    phi = torch.atan2(relative.imag, relative.real).to(dtype=distance_vals.dtype)
    if flip_phi:
        phi = -phi

    cos_terms = torch.cos(distance_vals.unsqueeze(1) * omega.unsqueeze(0) + phi.unsqueeze(0))
    return cos_terms @ amp_product
...
def main() -> None:
    ...
    (
        ...
        _k_mean_complex,
        k_complex,
    ) = mean_vector_product_and_phase(q_unrot, k_unrot)

    q_mean_abs = torch.abs(q_mean_complex).to(device=device, dtype=dtype)
    ...
    steps = max(2, steps)
    max_log = math.log10(float(max(distance_limit, 1))) if distance_limit > 1 else 0.0
    distance_vals = torch.logspace(0.0, max_log, steps=steps, device=device, dtype=dtype)
    ...
    random_reconstructions: list[tuple[int, torch.Tensor]] = []
    sample_count = min(3, k_complex.shape[0])
    if sample_count > 0:
        generator = torch.Generator()
        generator.manual_seed(args.seed)
        sampled_indices = torch.randperm(k_complex.shape[0], generator=generator)[
            :sample_count
        ].tolist()
        for token_idx in sampled_indices:
            k_vector = k_complex[token_idx]
            reconstruction = reconstruct_with_single_key(
                distance_vals,
                omega,
                q_mean_complex,
                q_mean_abs,
            ...
            random_reconstructions.append((token_idx, reconstruction.detach().cpu()))
