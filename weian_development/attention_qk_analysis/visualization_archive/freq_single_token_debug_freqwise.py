...
def main() -> None:
    ...
    window_limit = token_idx if args.fft_window is None else min(args.fft_window, token_idx)
    window = max(args.min_history, window_limit)
    if window < 1:
    ...
    hist_slice = k_block[hist_start:token_idx].to(torch.float32)
    hist_reversed = hist_slice.flip(0)
    hist_real = hist_reversed[:, :freq_count].contiguous()
    hist_imag = hist_reversed[:, freq_count:].contiguous()
    hist_complex = torch.complex(hist_real, hist_imag)

    q_token = q_block[token_idx].to(torch.float32)
    q_real = q_token[:freq_count].contiguous()
    q_imag = q_token[freq_count:].contiguous()
    q_complex = torch.complex(q_real, q_imag)

    contributions = q_complex.unsqueeze(0) * torch.conj(hist_complex)

    contrib_mean = contributions.mean(dim=0, keepdim=True)
    centered = contributions - contrib_mean

    omega = inv_freq[:freq_count].to(device=device, dtype=dtype)
    phase = deltas.unsqueeze(1) * omega.unsqueeze(0)
    exp_neg = torch.exp(-1j * phase)
    coeffs = (centered * exp_neg).sum(dim=0) / window

    exp_pos = torch.exp(1j * phase)
    recon_centered = exp_pos * coeffs.unsqueeze(0)
    reconstructed = (recon_centered + contrib_mean).real.sum(dim=1)

    raw_total = contributions.real.sum(dim=1)

    fig, ax = plt.subplots(1, 1, figsize=tuple(args.figsize), dpi=args.dpi)
    x_vals = deltas.detach().cpu().numpy()
    ax.plot(x_vals, raw_total.detach().cpu().numpy(), label="Raw Q·K", linewidth=1.2)
    ax.plot(x_vals, reconstructed.detach().cpu().numpy(), label="RoPE freq recon", linewidth=1.2)
