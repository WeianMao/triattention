...
from weian_development.attention_qk_analysis.freq_magnitude_plots import (
    magnitude_pairs,
    causal_product_average,
)
...
def phase_shift_from_rotated(
    ...
    positions = torch.arange(q_block.shape[0], device=q_block.device, dtype=omega.dtype)
    phase_grid = positions.unsqueeze(1) * omega.unsqueeze(0)

    cos_vals = torch.cos(phase_grid)
    sin_vals = torch.sin(phase_grid)
    exp_neg = torch.complex(cos_vals, -sin_vals)

    q_rot = q_complex * exp_neg
    k_rot = k_complex * exp_neg

    prefix_conj = torch.cumsum(torch.conj(k_rot), dim=0)
    accumulator = torch.sum(q_rot * prefix_conj, dim=0)

    return torch.atan2(accumulator.imag, accumulator.real)
...
def main() -> None:
    ...
    phi_hat = phase_shift_from_rotated(q_block, k_block, omega)
    if args.flip_phi:
        phi_hat = -phi_hat
    ...
    reconstructed_plain = cos_terms @ causal_mean[:freq_count]

    cos_terms_phi = torch.cos(distance_vals.unsqueeze(1) * omega.unsqueeze(0) + phi_hat[:freq_count].unsqueeze(0))
    reconstructed_phi = cos_terms_phi @ causal_mean[:freq_count]
