...
5. **图 4（Reconstructed Σ_f |Q||K| cos(ω_f Δ)）**：假设旋转角仅受距离影响，用图 3 的幅值作为权重，对 token 距离 Δ 的理论内积进行重构，观察距离衰减趋势。
6. **归一化**：幅值保持在线性域；第四图直接绘制加权余弦和，无额外对数或归一化。

## 输出说明
- 输出目录结构与注意力热图一致：`outputs/.../freq_magnitude_plots/qidXXXX_traceYY/layer_##_head_##_freq.png`。
- 每张图片包含四列子图，前三列的横轴为 RoPE 周期（对数刻度），最后一列的横轴为 token 距离 Δ；纵轴分别为对应的平均幅值或重构值。
...
- 图 3 的平均是在自回归掩码下对全部 (i, j) 对求和后除以对数，因此不会因序列长度而失真。
