...
9. **图 8（Ground-truth avg Q·K）**：直接对原始注意力预 softmax 的 Q·K 结果按 Δ 聚合，反映真实的相对位置得分。
10. **归一化**：幅值保持在线性域；曲线直接绘制加权余弦和或实际平均值，无额外对数或归一化。

## 输出说明
- 输出目录结构与注意力热图一致：`outputs/.../freq_magnitude_plots/qidXXXX_traceYY/layer_##_head_##_freq.png`。
- 每张图片包含八个子图，前五个的横轴为 RoPE 周期（对数刻度），后三个的横轴为 token 距离 Δ（同样为对数刻度）；纵轴分别为对应的平均幅值、角度统计、重构值或真实平均值。
