#!/usr/bin/env python3
"""Config-driven launcher for LazyEviction-native RKV (sharded, HF backend)."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "LazyEviction" / "weian_script" / "configs" / "rkv_lazy_aime.yaml"
MERGE_SCRIPT = PROJECT_ROOT / "weian_development" / "merge_rkv_lazy_shards.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-eval", action="store_true", help="Skip eval step (not wired yet, kept for parity)")
...
def compute_shard_questions(total_questions: int, num_shards: int, shard_id: int) -> tuple[int, int]:
...
def determine_gpus(args: argparse.Namespace, experiment: Dict) -> List[str]:
    ...
    if isinstance(cfg_gpus, str) and cfg_gpus.strip().lower() == "auto":
        ...
        return parse_gpu_string(",".join(str(item) for item in fallback))
        return []
    if isinstance(cfg_gpus, (list, tuple)):
        return parse_gpu_string(",".join(str(item) for item in cfg_gpus))
...
def resolve_path(value: str | Path) -> Path:
    ...
    if path.is_absolute():
        return path
    parts = path.parts
    if parts and parts[0] in {"LazyEviction", "weian_development"}:
        path = Path(*parts)
...
def launch_shard(
    ...
    log_path = log_dir / f"rkv_lazy_shard{shard_id:02d}.log"
...
def run_shards(
    ...
    if skip_existing:
        ...
        for shard_id in range(total_shards):
            _, shard_questions = compute_shard_questions(total_questions or 0, total_shards, shard_id)
            if shard_questions == 0:
                print(f"[skip] shard {shard_id} has 0 assigned questions, no output required.")
                continue
            missing = []
            run_id = 0
            if not run_completed(output_dir, shard_id, run_id, shard_questions):
                missing.append(run_id)
            if not missing:
                print(f"[skip] shard {shard_id} has existing outputs with expected records.")
                continue
    ...
    if dry_run:
        for shard_id in shards_to_run:
            ...
            log_path = (log_dir / f"rkv_lazy_shard{shard_id:02d}.log").resolve()
            cmd_preview = base_cmd + ["--shard_id", str(shard_id)]
            shard_questions = compute_shard_questions(total_questions or 0, total_shards, shard_id)[1]
            print(f"[dry-run] shard {shard_id} -> GPU {gpu} (questions={shard_questions})\n  log: {log_path}\n  cmd: {' '.join(cmd_preview)}")
        return
...
def main() -> None:
    ...
    log_dir = resolve_path(args.log_dir or experiment.get("log_dir", "logs/rkv_lazy"))
    method_output_dir = resolve_path(args.method_output_dir or experiment.get("method_output_dir", "outputs/rkv_lazy/shards"))
    ...
    runner_args["output_dir"] = resolve_path(args.output_dir or runner_args.get("output_dir", method_output_dir))
    runner_args["dataset_path"] = resolve_path(runner_args["dataset_path"])
    runner_args["model_path"] = resolve_path(runner_args["model_path"])
    runner_args["tokenizer_path"] = resolve_path(runner_args.get("tokenizer_path", runner_args["model_path"]))
    ...
    num_samples = int(runner_args.get("num_samples", 1))
    ...
    merge_outputs(Path(runner_args["output_dir"]), merged_dir_name, args.skip_merge, args.dry_run)
    if not args.no_eval:
        print("[info] Eval step skipped (mirrors run_sparse_prefill_keep_sharded_eval.sh); run downstream eval manually if needed.")
