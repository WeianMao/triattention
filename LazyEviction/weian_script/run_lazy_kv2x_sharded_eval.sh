...
for gpu in ${GPUS}; do
    ...
    conda run -n lazy_evict bash -lc "nohup env CUDA_VISIBLE_DEVICES=${gpu} NUM_SHARDS=${NUM_SHARDS} SHARD_ID=${shard_id} bash ${LAZY_DIR}/eval_qwen_aime_sharded_kv2x.sh > ${log_path} 2>&1 &"
    shard_id=$((shard_id + 1))
done
