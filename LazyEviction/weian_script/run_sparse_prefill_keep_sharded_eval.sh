...
# Launch SparseRound (prefill-keep) AIME evaluation across multiple GPUs.

SCRIPT_DIR="$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
LAZY_DIR="$( dirname "${SCRIPT_DIR}" )"
REPO_DIR="$( dirname "${LAZY_DIR}" )"
LOG_DIR="${REPO_DIR}/logs/lazy_eviction_sparse_round_prefill"
mkdir -p "${LOG_DIR}"
...
EVAL_SCRIPT="${LAZY_DIR}/eval_qwen_aime_sparse_prefill_keep_sharded.sh"
...
for gpu in ${GPUS}; do
    ...
    log_path="${LOG_DIR}/sparse_prefill_shard${shard_id}.log"
    echo "Launching SparseRound-Prefix shard ${shard_id}/${NUM_SHARDS} on GPU ${gpu}, logging to ${log_path}"
