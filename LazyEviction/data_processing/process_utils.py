...
def process_gpqa_test(item):
    """
    处理GPQA数据集，将其转换为对话格式
    GPQA是多选题科学问题数据集，包含物理、化学、生物等学科
    """
    question = item['question']
    
    # 确保问题已经包含了选项和指示
    # GPQA数据集的问题已经格式化好了，包含选项和最后的指示
    ...
    sample = {
        ...
        'answer': item['answer']  # A, B, C, D 中的一个
    }
    yield sample

def process_commonsense_qa_test(item):
    """
    处理CommonsenseQA数据集，将其转换为对话格式
    CommonsenseQA是一个多选题数据集，包含常识推理问题
    """
    question = item['question']
    
    # 确保问题已经包含了选项和指示
    # CommonsenseQA数据集的问题已经格式化好了，包含选项和最后的指示
    
    sample = {
        'dataset': 'commonsense_qa-cot',
        'id': item['id'],
        'messages': [
            {'role': 'user', 'content': question},
            {'role': 'assistant', 'content': ''}  # 留空，让模型生成
