...
def extract_char_answer(pred_str, exhaust=False):
    pred = []
    if 'final answer is $' in pred_str and '$. I hope' in pred_str:
        tmp = pred_str.split('final answer is $', 1)[1]
        pred = [tmp.split('$. I hope', 1)[0].strip()]
        ...
        pred = [pred_str.split('he answer is')[-1].strip()]
    # elif ('nswer is (' in pred_str):
    #     pred = [pred_str.split('nswer is (')[-1].strip()]
    # elif ('nswer is option' in pred_str):
    #     pred = [pred_str.split('nswer is option')[-1].strip()]
    else:
        program_output = extract_program_output(pred_str)
        if program_output != "":
            # fall back to program
            ...
            pattern = '[A-Z]'
            ans = re.findall(pattern, pred_str)
            if(len(ans) >= 1):
                ans = ans[-1]
            else:
                ans = ''
    ...
    for ans in pred:
        # 检查是否是单个大写字母，若是则直接添加
        if len(ans.strip()) == 1 and 'A' <= ans.strip() <= 'Z':
            _pred.append(ans.strip())
        else:
            # 检查处理后的答案是否包含大写字母
            uppercase_letters = re.findall(r'[A-Z]', ans)
            if uppercase_letters:
                # 只取第一个大写字母
                _pred.append(uppercase_letters[0])
            else:
                # 保留原始处理逻辑作为兜底
                ans = ans.strip().split("\n")[0]
                ans = ans.lstrip(":")
    ...
    if exhaust:
        ...
        return _pred[0] if _pred else ""  # 返回第一个结果而不是最后一个
...
def extract_last_single_char_answer(question, reasoning, task):
    return extract_char_answer(reasoning, exhaust=False)
