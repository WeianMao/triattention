...
def main():
    ...
    parser.add_argument('--max_model_len', type=int, default=None,
                        help='vLLM `max_model_len`，默认跟随 --max_tokens')
    ...
    parser.add_argument('--serializer',
                        ...
                        help='结果序列化方式（默认 msgpack+gzip）')
    parser.add_argument('--compression_level', type=int, default=3,
                        help='压缩等级（gzip/zstd 模式生效，范围 1-9 或 zstd 对应等级）')
    parser.add_argument('--gpu_memory_utilization', type=float, default=None,
                        help='可选覆盖 vLLM 的 gpu_memory_utilization 参数')
    ...
    deep_llm = DeepThinkLLM(model=args.model, **llm_kwargs)
    ...
    if args.serializer == 'pickle':
        ...
        try:
            dump_msgpack(result_data, str(result_path),
