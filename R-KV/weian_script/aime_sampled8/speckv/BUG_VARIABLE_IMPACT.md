...
```
疑问：第3题的 Δ 是只有第3题的 prefill 长度，还是第1+2+3题的累加？

答案：只是第3题自己的 prefill 长度，不会累加！

原因分析：

第2题结束时：
  - absolute_position = P1 + D1 + D2
    （注意：P2 没加进去，因为 prefill 时走的是截断分支，不更新 absolute_position）
  - cache_positions 包含第2题 decode 阶段的位置
  - 末尾是近似连续的：[..., P1+D1+D2-100, P1+D1+D2-99, ..., P1+D1+D2-1]

第3题 prefill P3 个 token：
  - 实际编码位置 = [P1+D1+D2, P1+D1+D2+1, ..., P1+D1+D2+P3-1]
  - cache_positions 被截断成最后 P3 个：
    [P1+D1+D2-P3, P1+D1+D2-P3+1, ..., P1+D1+D2-1]

  Δ[j] = (P1+D1+D2+j) - (P1+D1+D2-P3+j) = P3  ← 只是 P3！

关键点：
  1. cache_positions 末尾总是近似连续的（因为 decode 阶段是逐个追加 absolute_position）
  2. 截断后取的是末尾的连续位置
  3. 实际编码位置也是从 absolute_position 开始连续的
  4. 所以差值 = 截断长度 = 当前题目的 prefill 长度

无论第几题：
  Δ ≈ 当前题目的 prefill 长度 ≈ 100-400 tokens
  不会累加！
```

### 实验数据验证

| 题目索引 | Δ (实测) | prefill_len | 是否匹配 |
|---------|---------|-------------|---------|
| 1 | 160 | 159 | ✓ |
| 2 | 129 | 129 | ✓ |
| 3 | 134 | 134 | ✓ |
...
所有 30 道题的 Δ 都精确等于当前题目的 prefill 长度。
